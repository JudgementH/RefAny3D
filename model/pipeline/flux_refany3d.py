...
class FluxPipelineOutput(BaseOutput):
    ...
    coords: Union[List[PIL.Image.Image], np.ndarray]
...
def convert_to_condition(
    ...
    if condition_type == "depth":
        ...
        condition_image = raw_img.convert("RGB").filter(ImageFilter.GaussianBlur(blur_radius)).convert("RGB")
...
class Condition(object):
    def __init__(
        ...
        self.latent_mask = latent_mask.T.reshape(-1) if latent_mask is not None else None
        self.is_complement = is_complement

    def encode(self, pipe: FluxPipeline, empty: bool = False) -> Tuple[torch.Tensor, torch.Tensor, int]:
...
def specify_lora(lora_modules: List[BaseTunerLayer], specified_lora):
    if specified_lora is not None and "+" in specified_lora:
        # If specified_lora is a combination, split it into individual adapters
        specified_lora = specified_lora.split("+")
    else:
    ...
    for module in valid_lora_modules:
        for adapter in module.active_adapters:
            if adapter in module.scaling:
                ...
                module.scaling[adapter] = 1 if adapter in specified_lora else 0
    try:
        yield
    finally:
...
def attn_forward(
    ...
    **kwargs: dict,
    ...
    if "cache_attn_map" in kwargs:
        kwargs["cache_attn_map"][attn.cache_idx] = []
    attn_outputs = []
    for i, query in enumerate(queries):
        ...
        if "cache_attn_map" in kwargs and hasattr(attn, "cache_idx") and i > 0:
            h = kwargs["h"]
            w = kwargs["w"]
            key = torch.cat(keys_, dim=2)
            value = torch.cat(values_, dim=2)
            attn_output, attention_probs = scaled_dot_product_attention(
            ...
            kwargs["cache_attn_map"][attn.cache_idx].append(attn_map.detach().cpu())
        else:
            attn_output = F.scaled_dot_product_attention(query, torch.cat(keys_, dim=2), torch.cat(values_, dim=2)).to(
                query.dtype
            )
            # if i == 1:
            #     # rgb image branch
            #     n_condition = 3
            #     coord_query = queries[i + 1]
            #     coord_keys = keys[-n_condition:]
            #     rgb_values = values[-n_condition * 2 : -n_condition]

            #     additional_attn_output = F.scaled_dot_product_attention(
            #         coord_query, torch.cat(coord_keys, dim=2), torch.cat(rgb_values, dim=2)
            #     )
            #     gate = torch.nn.functional.sigmoid(attn.add_gate_proj(hidden_states[1]))
            #     # gate = gate.reshape(bs, -1, attn.heads, 1)
            #     gate = gate.reshape(bs, -1, 1, 1)
            #     gate = gate.transpose(1, 2)
            #     attn_output = attn_output + gate * additional_attn_output
...
def block_forward(
    ...
    **kwargs: dict,
    ...
    for i in range(len(text_hidden_states)):
        ...
        norm_h = self.norm2_context(text_h) * (1 + scale_mlp[:, None]) + shift_mlp[:, None]
    ...
    for i in range(len(image_hidden_states)):
        ...
        image_h = (image_hidden_states[i] + img_attn_output[i] * gate_msa.unsqueeze(1)).to(image_hidden_states[i].dtype)
...
def single_block_forward(
    ...
    **kwargs: dict,
    ...
    attn_outputs = attn_forward(self.attn, hidden_state_norm, adapters, position_embs=position_embs, **kwargs)
...
def transformer_forward(
    ...
    **kwargs: dict,
    ...
    if getattr(self, "aux_token", False):
        condition_num = len(image_features) // 2 - 1
        # domain_tags = [0, 1] + condition_num * [0] + condition_num * [1]
        domain_tags = [0, 1] + [0 for _ in range(condition_num)] + [1 for _ in range(condition_num)]
        domain_tags = torch.tensor(domain_tags, dtype=torch.long, device=image_hidden_states[0].device)
        # domain_tags = torch.tensor([0, 1, 0, 0, 0, 1, 1, 1], dtype=torch.long, device=image_hidden_states[0].device)
        domain_y = self.domain_embedder(domain_tags)
        for i, temb in enumerate(tembs[txt_n:]):
            tembs[i + txt_n] = temb + domain_y[i]

        # domain_tags = torch.tensor([0, 0, 0, 0], dtype=torch.long, device=image_hidden_states[0].device)
        # domain_tags = torch.tensor([0], dtype=torch.long, device=image_hidden_states[0].device)
        # domain_y = self.domain_embedder(domain_tags)
        # tembs[2] = tembs[2] + domain_y[0]
    ...
    gckpt_kwargs: Dict[str, Any] = {"use_reentrant": False} if is_torch_version(">=", "1.11.0") else {}
    ...
    for block in self.single_transformer_blocks:
        ...
        if self.training and self.gradient_checkpointing:
            all_hidden_states = torch.utils.checkpoint.checkpoint(single_block_forward, **block_kwargs, **gckpt_kwargs)
        else:
    ...
    coord_image_hidden_states = self.norm_out(all_hidden_states[txt_n + 1], tembs[txt_n + 1])
    coord_output = self.proj_out(coord_image_hidden_states)
...
def generate(
    ...
    group_mask: List[List[int]] = None,
    coord_position_deltas: Optional[List] = [0, 0],
    ...
    **params: dict,
    ...
    coord_latents, coord_latent_image_ids = self.prepare_latents(
        batch_size * num_images_per_prompt,
        num_channels_latents,
        height,
        width,
    ...
    # coord_latents, coord_latent_image_ids = latents.clone(), latent_image_ids.clone()
    coord_latent_image_ids[:, 1] += coord_position_deltas[0]
    coord_latent_image_ids[:, 2] += coord_position_deltas[1]
    ...
    if latent_mask is not None:
        ...
        coord_latents = coord_latents[:, latent_mask]
        coord_latent_image_ids = coord_latent_image_ids[latent_mask]
    ...
    branch_n = len(conditions) + 3
    group_mask = group_mask.detach().clone().to(self.device, dtype=torch.bool)
    # group_mask = torch.tensor(group_mask.detach().clone(), dtype=torch.bool).to(self.device)
    # group_mask = torch.ones([branch_n, branch_n], dtype=torch.bool)
    # # Disable the attention cross different condition branches
    # # group_mask[3:, 3:] = torch.diag(torch.tensor([1] * len(conditions)))

    # # group_mask[3:, 3:] = torch.diag(torch.tensor([1] * len(conditions)))
    # group_mask[3:9, 3:9] = torch.diag(torch.tensor([1] * (len(conditions))))
    # group_mask[3:6, 6:9] = torch.diag(torch.tensor([1] * (len(conditions) // 2)))
    # group_mask[6:9, 3:6] = torch.diag(torch.tensor([1] * (len(conditions) // 2)))
    # group_mask[6:9, 6:9] = torch.diag(torch.tensor([1] * (len(conditions) // 2)))
    # group_mask[1, 6:] = False  # Disable the attention from Image branch to condition branches
    # # group_mask[2, 0] = False  # Disable the attention from Coord branch to Text branch
    # group_mask[2, 3:6] = False  # Disable the attention from Coord branch to RGB condition branches

    # Disable the attention from condition branches to image branch and text branch
    if kv_cache:
        group_mask[3:, :3] = False

    # add hidden h w
    transformer_kwargs["h"] = 2 * (int(height) // (self.vae_scale_factor * 2)) // 2
    transformer_kwargs["w"] = 2 * (int(width) // (self.vae_scale_factor * 2)) // 2

    # Denoising loop
    self.coord_scheduler = copy.copy(self.scheduler)
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            # save attn map
            if "attn_map" in transformer_kwargs and i == transformer_kwargs["step"]:
                transformer_kwargs["cache_attn_map"] = {}
            transformer_outputs = transformer_forward(
                self.transformer,
                image_features=[latents, coord_latents] + (c_latents if use_cond else []),
                text_features=[prompt_embeds],
                img_ids=[latent_image_ids, coord_latent_image_ids] + (c_ids if use_cond else []),
                txt_ids=[text_ids],
                timesteps=[timestep, timestep, timestep] + (c_timesteps if use_cond else []),
                pooled_projections=[pooled_prompt_embeds] * 3 + (c_projections if use_cond else []),
                guidances=[guidance] * 3 + (c_guidances if use_cond else []),
                return_dict=False,
                adapters=[main_adapter] * 2 + ["coord"] + (c_adapters if use_cond else []),
            ...
            # save attn map
            if "attn_map" in transformer_kwargs and i == transformer_kwargs["step"]:
                transformer_kwargs["attn_map"] = transformer_kwargs["cache_attn_map"]
                transformer_kwargs.pop("cache_attn_map")
            rgb_noise_pred, coord_noise_pred = transformer_outputs[:2]

            if image_guidance_scale != 1.0:
                unc_transformer_outputs = transformer_forward(
                    self.transformer,
                    image_features=[latents, coord_latents] + (uc_latents if use_cond else []),
                    text_features=[prompt_embeds],
                    img_ids=[latent_image_ids, coord_latent_image_ids] + (c_ids if use_cond else []),
                    txt_ids=[text_ids],
                    timesteps=[timestep, timestep, timestep] + (c_timesteps if use_cond else []),
                ...
                unc_rgb_noise_pred, unc_coord_noise_pred = unc_transformer_outputs[:2]

                rgb_noise_pred = unc_rgb_noise_pred + image_guidance_scale * (rgb_noise_pred - unc_rgb_noise_pred)
                coord_noise_pred = unc_coord_noise_pred + image_guidance_scale * (
                    coord_noise_pred - unc_coord_noise_pred
                )

            # compute the previous noisy sample x_t -> x_t-1
            latents_dtype = latents.dtype
            latents = self.scheduler.step(rgb_noise_pred, t, latents)[0]
            coord_latents = self.coord_scheduler.step(coord_noise_pred, t, coord_latents)[0]

            if latents.dtype != latents_dtype:
                if torch.backends.mps.is_available():
                    # some platforms (eg. apple mps) misbehave due to a pytorch bug: https://github.com/pytorch/pytorch/pull/99272
                    latents = latents.to(latents_dtype)
            if coord_latents.dtype != latents_dtype:
                if torch.backends.mps.is_available():
                    # some platforms (eg. apple mps) misbehave due to a pytorch bug:
                    coord_latents = coord_latents.to(latents_dtype)
    ...
    if output_type == "latent":
        ...
        coord_latents = self._unpack_latents(coord_latents, height, width, self.vae_scale_factor)
        coord_latents = (coord_latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor
        coord = self.vae.decode(coord_latents, return_dict=False)[0]
        coord = self.image_processor.postprocess(coord, output_type=output_type)
    ...
    return FluxPipelineOutput(images=image, coords=coord)
